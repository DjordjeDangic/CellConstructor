...
class ThermalConductivity:
        ...
        if(not self.off_diag):
            if(temp_key in self.lifetimes.keys() and temp_key in self.lineshapes.keys()):
                if(prefered == 'lineshape'):
                    ...
                    spec_kappa = self.calc_spectral_kappa_gk_diag(temperature)
                else:
                    print('Both lineshapes and lifetimes are calculated. Calculating spectral kappa from lifetimes!')
                    spec_kappa = self.calc_spectral_kappa_srta_diag(temperature, ne)
            ...
            if(temp_key in self.lifetimes.keys() and temp_key in self.lineshapes.keys()):
                if(prefered == 'lineshape'):
                    ...
                    print('Calculation with offdiagonal elements have been initialized. Not possible to calculate spectral kappa with lifetimes! ')
                    raise RuntimeError('Calculation with offdiagonal elements have been initialized. Not possible to calculate spectral kappa with lifetimes! ')
                    spec_kappa = (np.zeros(ne), np.zeros(ne))
   ...
   ####################################################################################################################################
    ...
    def calculate_kappa(self, temperatures = [300.0], write_lifetimes = True, mode = 'SRTA', offdiag_mode = 'wigner',  gauss_smearing = False, lf_method = 'fortran-LA', isotope_scattering = False, isotopes = None, \
        ...
        offdiag_mode     : How to treat the off diagonal terms. 'isaeva' - Isaeva et al. Nat. Comm., 'wigner' - Simoncelli et al. Nature, 'perturbative'
        ...
        for itemp in range(ntemp):
            ...
            if(mode == 'SRTA'):
                if(not self.off_diag):
                    kappa, _ = self.calculate_kappa_srta_diag(temperatures[itemp], ne, write_lifetimes, gauss_smearing = gauss_smearing, isotope_scattering=isotope_scattering, isotopes= isotopes, lf_method = lf_method)
...
#        print(integrands_plus.shape)
        ...
        kappa = np.zeros((3,3))
        per_mode = np.zeros((self.nkpt, self.nband, 3, 3))
        for istar in self.qstar:
            for iqpt in istar:
                for iband in range(self.nband):
                    if(self.freqs[iqpt, iband] != 0.0):
                        ...
                        for r in self.rotations:
                            ...
                            gvel_sum += np.outer(gvel.conj(), gvel)
                        gvel_sum = gvel_sum.real*SSCHA_TO_MS**2/float(len(self.rotations))
                        per_mode[iqpt, iband] = gvel_sum*integrals[iqpt][iband]*self.freqs[iqpt][iband]**2
                        per_mode[iqpt, iband] += per_mode[iqpt, iband].T
        ...
        per_mode = per_mode/2.0*HBAR_JS**2/KB/temperature**2/self.volume/float(self.nkpt)*1.0e30*np.pi

        return kappa, per_mode
            ...
            for iqpt in istar:
                for iband in range(self.nband):
                    if(self.freqs[iqpt, iband] != 0.0):
                        ...
                        #c_matrix1 = (1.0 + (self.freqs[iqpt,iband] + energies)/self.freqs[iqpt,iband])*exp_omega1*np.exp(exponents)/(exp_omega1*np.exp(exponents) - 1.0)/(exp_omega1 - 1.0)
                        ...
                        for jband in range(self.nband):
                            if(self.freqs[iqpt, jband] != 0.0):
                                ...
                                #c_matrix3 = (1.0 + self.freqs[iqpt,jband]/(self.freqs[iqpt,jband] + energies))*exp_omega3/(exp_omega3 - 1.0)/(exp_omega3*np.exp(exponents) - 1.0)
                                #c_matrix4 = (1.0 + self.freqs[iqpt,jband]/(self.freqs[iqpt,jband] + energies))*exp_omega4/(exp_omega4 - 1.0)/(exp_omega4*np.exp(-1.0*exponents) - 1.0)
                                c_matrix1 = (1.0 + self.freqs[iqpt,jband]/self.freqs[iqpt,iband])*exp_omega3/(exp_omega3 - 1.0)/(exp_omega1 - 1.0)
                                c_matrix2 = (1.0 - self.freqs[iqpt,jband]/self.freqs[iqpt,iband])*exp_omega3/(exp_omega3 - 1.0)/(exp_omega2 - 1.0)
                                c_matrix3 = (1.0 - self.freqs[iqpt,jband]/self.freqs[iqpt,iband])*exp_omega4/(exp_omega4 - 1.0)/(exp_omega1 - 1.0)
                                c_matrix4 = (1.0 + self.freqs[iqpt,jband]/self.freqs[iqpt,iband])*exp_omega4/(exp_omega4 - 1.0)/(exp_omega2 - 1.0)
                                gvel = np.zeros_like(self.gvels[iqpt, iband, jband])
                                ...
                                gamma_matrix1 = gamma_sum/(gamma_sum**2 + (freq_diff - energies)**2)
                                gamma_matrix2 = gamma_sum/(gamma_sum**2 + (freq_sum + energies)**2)
                                gamma_matrix3 = gamma_sum/(gamma_sum**2 + (freq_sum - energies)**2)
                                gamma_matrix4 = gamma_sum/(gamma_sum**2 + (freq_diff + energies)**2)
                                if(iband != jband):
                                    ac_kappa_offdiag += (np.einsum('ij,k->kij', gvel_sum,front_factor*c_matrix1*gamma_matrix1) + \
                                            np.einsum('ij,k->kij', gvel_sum,front_factor*c_matrix2*gamma_matrix2) + \
                                            np.einsum('ij,k->kij', gvel_sum,front_factor*c_matrix3*gamma_matrix3) + \
                                            np.einsum('ij,k->kij', gvel_sum,front_factor*c_matrix4*gamma_matrix4))*self.freqs[iqpt, iband]*self.freqs[iqpt, jband]/4.0
                                else:
                                    ac_kappa_diag += (np.einsum('ij,k->kij', gvel_sum,front_factor*c_matrix1*gamma_matrix1) + \
                                            np.einsum('ij,k->kij', gvel_sum,front_factor*c_matrix2*gamma_matrix2) + \
                                            np.einsum('ij,k->kij', gvel_sum,front_factor*c_matrix3*gamma_matrix3) + \
                                            np.einsum('ij,k->kij', gvel_sum,front_factor*c_matrix4*gamma_matrix4))*self.freqs[iqpt, iband]*self.freqs[iqpt, jband]/4.0
    ...
    def calculate_kappa_perturb_AC_offdiag_v2(self, temperature, energies, gauss_smearing = False, isotope_scattering = False, isotopes = None, lf_method = 'fortran-LA'):
                    ...
                    if(self.freqs[iqpt, iband] != 0.0):
                        ...
                        for jband in range(iband, self.nband):
                            if(self.freqs[iqpt, jband] != 0.0):
                                ...
                                exp_omega4 = np.exp(-1.0*omega_exponents[iqpt,jband])
                                if(freq_diff == 0.0):
                                    front_factor1 = 1.0 #np.ones_like(exponents, dtype=float)
                                    front_factor4 = 1.0 #np.ones_like(exponents, dtype=float)
                                else:
                                    front_factor1 = (np.exp(freq_diff*SSCHA_TO_THZ*1.0e12*HPLANCK/KB/temperature) - 1.0)/(freq_diff*SSCHA_TO_THZ*1.0e12*HPLANCK/KB/temperature) 
                                    #np.array([(np.exp(freq_diff*SSCHA_TO_THZ*1.0e12*HPLANCK/KB/temperature) - 1.0)/(freq_diff*SSCHA_TO_THZ*1.0e12*HPLANCK/KB/temperature) for x in range(ne)])
                                    front_factor4 = -(np.exp(-freq_diff*SSCHA_TO_THZ*1.0e12*HPLANCK/KB/temperature) - 1.0)/(freq_diff*SSCHA_TO_THZ*1.0e12*HPLANCK/KB/temperature) 
                                    #-np.array([(np.exp(-freq_diff*SSCHA_TO_THZ*1.0e12*HPLANCK/KB/temperature) - 1.0)/(freq_diff*SSCHA_TO_THZ*1.0e12*HPLANCK/KB/temperature) for x in range(ne)])
                                front_factor3 = (np.exp(freq_sum*SSCHA_TO_THZ*1.0e12*HPLANCK/KB/temperature) - 1.0)/(freq_sum*SSCHA_TO_THZ*1.0e12*HPLANCK/KB/temperature)
                                #np.array([(np.exp(freq_sum*SSCHA_TO_THZ*1.0e12*HPLANCK/KB/temperature) - 1.0)/(freq_sum*SSCHA_TO_THZ*1.0e12*HPLANCK/KB/temperature) for x in range(ne)])
                                front_factor2 = (np.exp(-freq_sum*SSCHA_TO_THZ*1.0e12*HPLANCK/KB/temperature) - 1.0)/(freq_sum*SSCHA_TO_THZ*1.0e12*HPLANCK/KB/temperature)
                                #-np.array([(np.exp(-freq_sum*SSCHA_TO_THZ*1.0e12*HPLANCK/KB/temperature) - 1.0)/(freq_sum*SSCHA_TO_THZ*1.0e12*HPLANCK/KB/temperature) for x in range(ne)])
                                ...
                                c_matrix4 = freq_diff**2/self.freqs[iqpt,jband]/self.freqs[iqpt,iband]/(exp_omega1 - 1.0)/(exp_omega3 - 1.0)
                                gvel = np.zeros_like(self.gvels[iqpt, iband, jband])
                                gvel_sum = np.zeros_like(ac_kappa_diag[0,:,:], dtype=complex)
                                for r in self.rotations:
                                    rot_q = np.dot(self.reciprocal_lattice.T, np.dot(r.T, np.linalg.inv(self.reciprocal_lattice.T)))
    ...
    def get_lineshapes(self, temperature, write_lineshapes, energies, method = 'fortran', mode_mixing = 'no', gauss_smearing = False, lorentzian_approximation = False):
        ...
        if(method == 'python'):
            ...
            for ikpt in range(self.nirrkpt):
                jkpt = self.qstar[ikpt][0]
                if(mode_mixing == 'no' and not lorentzian_approximation):
                    if(CC.Methods.is_gamma(self.dyn.structure.unit_cell, self.k_points[jkpt])):
                        for iband in range(self.nband):
                            if(self.freqs[jkpt, iband] < np.amax(self.freqs[jkpt])*1.0e-6):
                ...
                for iband in range(self.nband):
                    if(self.freqs[iqpt, iband] != 0.0):
                        ...
                        for r in self.rotations:
                            ...
                            gvel_sum += np.outer(gvel.conj(), gvel)
                        gvel_sum = gvel_sum.real/float(len(self.rotations))
                        per_mode[iqpt, iband] = self.cp[cp_key][iqpt][iband]*gvel_sum*self.lifetimes[lf_key][iqpt][iband]
                        per_mode[iqpt, iband] += per_mode[iqpt, iband].T
                        kappa += self.cp[cp_key][iqpt][iband]*gvel_sum*self.lifetimes[lf_key][iqpt][iband]
        kappa += kappa.T
        kappa = kappa/2.0*SSCHA_TO_MS**2
        per_mode = per_mode/2.0*SSCHA_TO_MS**2/self.volume/float(self.nkpt)*1.0e30
    ...
    def calculate_kappa_srta_offdiag_perturbative(self, temperature, ne, write_lifetimes, gauss_smearing = False, isotope_scattering = False, isotopes = None, lf_method = 'fortran-LA'):
        ...
        """
        Calculates both diagonal and off diagonal contribution to the lattice thermal conductivity in the perturbative limit ().
                    ...
                    if(self.freqs[iqpt, iband] != 0.0 and scatt_rates[iqpt, iband] != 0.0):
                        for jband in range(self.nband):
                            ...
                            if(self.freqs[iqpt, jband] != 0.0 and scatt_rates[iqpt, jband] != 0.0 and iband != jband):
                                ...
                                a3 = 0.5*(scatt_rates[iqpt, iband] + scatt_rates[iqpt, jband])
                                a4 = (self.freqs[iqpt,iband] - self.freqs[iqpt,jband])**2 + (scatt_rates[iqpt, iband] + scatt_rates[iqpt, jband])**2/4.0
                                kappa_diag += a1*a3/a4*gvel_sum/2.0/np.pi/float(len(self.rotations))/2.0#*float(len(istar))
